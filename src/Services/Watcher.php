...
class Watcher
{
    ...
    private function loadProjects()
    {
        foreach (Config::get('pragmarx/ci::projects') as $name => $data) {
            $project = $this->dataRepository->createOrUpdateProject($name,
                $data[ 'path' ], $data[ 'tests_path' ]);

            foreach ($data[ 'suites' ] as $suite_name => $suite_data) {
                $this->dataRepository->createOrUpdateSuite($name, $project->id,
                    $suite_data);
            }
    ...
    private function watch()
    {
        ...
        foreach ($this->watchFolders as $folder) {
            ...
            $this->listeners[ $folder ]->anything(function (
                $event,
                $resource,
                $path
            ) use ($me) {
